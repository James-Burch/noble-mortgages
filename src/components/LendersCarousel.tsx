...
const LendersCarousel: React.FC = () => {
  ...
  return (
    <section className={styles.section} aria-labelledby="lenders-heading">
      <div className={styles.container}>
        <h2 id="lenders-heading" className={styles.title}>
        ...
        <div
          ...
          role="marquee"
          aria-label="Partner lender and insurance provider logos"
        >
            ...
            {duplicatedLenders.map((lender, index) => (
              <div key={`${lender.id}-${index}`} className={styles.slide}>
                <img
                  ...
                  loading="lazy"
                />
                <span className={styles.logoLabel}>{lender.name}</span>
              </div>
            ))}
          </div>
        </div>
